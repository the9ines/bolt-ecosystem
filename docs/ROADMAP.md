...
| R26 | Hosted `localbolt.app` same-LAN discovery can miss real LAN peers when devices appear under different effective client IPs | Medium | Open | Observed symptom: two tabs on the same computer discover each other, but iPhone and Mac on the same network may not. Current hosted rendezvous scoping is effective-IP based, while iOS privacy, VPN, carrier, or network path behavior can make same-Wi-Fi devices appear in different rooms. 2026-05-16 mitigation kept: browser LAN ICE policy now rejects public host candidates in addition to srflx/relay candidates. 2026-05-16 rollback: the proposed visible manual peer-code UI fallback was reverted because UI changes were not approved; `localbolt.app` is verified back on bundle `index-BHBJLrf0.js` with no peer-code entry UI. `bolt-rendezvous` still contains `manual_signal`, but it is not product-active in the hosted LocalBolt web UI. Remaining resolution: diagnose and fix automatic iPhone/Mac same-Wi-Fi discovery without user-facing UI changes. |
