...
| R24 | CI/deploy inheritance drift across the three app repos | Medium | Mitigating | 2026-05-16 audit note: inheritance model is sound, but actual workflows are uneven. First mitigation pass added `localbolt-app` forward native CI gates (`native/shared` Rust bridge + macOS SwiftUI shell) and converted stale Tauri release/Windows surfaces into retired-path guards. Remaining follow-up: verify hosted/deploy surfaces (`localbolt-v3` Netlify/Fly.io, `localbolt` self-host package path) against `ARCHITECTURE.md` CI/Deploy Inheritance Map. |
