...
| R26 | Hosted `localbolt.app` same-LAN discovery can miss real LAN peers when devices appear under different effective client IPs | Medium | Mitigating | Observed symptom: two tabs on the same computer discover each other, but iPhone and Mac on the same network may not. Current hosted rendezvous scoping is effective-IP based, while iOS privacy, VPN, carrier, or network path behavior can make same-Wi-Fi devices appear in different rooms. 2026-05-16 mitigation: browser LAN ICE policy now rejects public host candidates in addition to srflx/relay candidates, keeping any future manual/QR peer-code fallback from becoming an internet path. Remaining resolution should preserve LAN-only product scope: diagnose room assignment, keep automatic hosted discovery limited to same effective-IP rooms, and add a LAN-safe fallback such as manual/QR peer-code signaling rather than broad internet discovery. |
