...
| R26 | Hosted `localbolt.app` same-LAN discovery can miss real LAN peers when devices appear under different effective client IPs | Medium | Mitigating | Observed symptom: two tabs on the same computer discover each other, but iPhone and Mac on the same network may not. Current hosted rendezvous scoping is effective-IP based, while iOS privacy, VPN, carrier, or network path behavior can make same-Wi-Fi devices appear in different rooms. 2026-05-16 mitigations: browser LAN ICE policy now rejects public host candidates in addition to srflx/relay candidates; rendezvous protocol adds explicit `manual_signal` routing for exact peer-code fallback without adding cross-room peers to automatic discovery; LocalBolt web UI exposes peer-code entry as a fallback. This keeps automatic discovery effective-IP scoped while giving real same-LAN users a recovery path when privacy/network behavior splits rooms. Deployment evidence: `localbolt-v3` CI green at `cf0afa8`; `bolt-rendezvous` CI green at `2500582`; Fly deployment verified live at `bolt-rendezvous.fly.dev`; `localbolt.app` verified serving bundle `index-C5M8AxAo.js` with static shell and LAN-only copy; live WebSocket smoke test confirmed `manual_signal` relay. Remaining resolution: validate the real iPhone/Mac same-Wi-Fi case before closing. |
