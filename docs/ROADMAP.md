...
| R25 | LocalBolt docs and website copy implied cross-network/internet discovery despite LAN-only governance | Medium | Mitigating | 2026-05-16 correction pass started: visible web copy, JSON-LD, PRDs, README/STATE docs, and architecture wording realigned to LAN-only LocalBolt. Browser WebRTC runtime hardening is in progress: shared `localbolt-browser` now removes STUN/TURN configuration and rejects non-host ICE candidates. Self-host CI/package hygiene improved: `localbolt` transport version guard now matches `@the9ines/bolt-transport-web@0.7.14` and counts real installed package paths instead of duplicate dependency edges. Migration prep started: `localbolt-browser` now re-exports the legacy surface self-host uses (`AppStore`, format helpers, `escapeHTML`) and has npmjs.org publish prep for `@the9ines/localbolt-browser@0.1.1` with `dist`/`wasm` package contents and a manual npm registry workflow. Remaining follow-up: publish `@the9ines/localbolt-browser` to npmjs.org, migrate self-host `localbolt` from the legacy `@the9ines/bolt-transport-web` package, and enforce strict discovery visibility, because hosted rendezvous can enforce same effective IP grouping but cannot by itself prove true LAN membership. |
