...
candidate policy and must not introduce STUN/TURN relay behavior. The current
...
| R26 | Hosted `localbolt.app` same-LAN discovery can miss real LAN peers when devices appear under different effective client IPs | Medium | Open | Observed symptom: two tabs on the same computer discover each other, but iPhone and Mac on the same network may not. Current hosted rendezvous scoping is effective-IP based, while iOS privacy, VPN, carrier, or network path behavior can make same-Wi-Fi devices appear in different rooms. Resolution should preserve LAN-only product scope: diagnose room assignment, keep automatic hosted discovery limited to same effective-IP rooms, and add a LAN-safe fallback such as manual/QR peer-code signaling with host-only ICE enforcement rather than broad internet discovery. |
