...
| R24 | CI/deploy inheritance drift across the three app repos | Medium | Mitigating | 2026-05-16 audit note: inheritance model is sound, but actual workflows were uneven. Mitigations landed: `localbolt-app` forward native CI gates (`native/shared` Rust bridge + macOS SwiftUI shell), retired Tauri release/Windows guards, `localbolt-v3` Tauri removed from active npm workspace/scripts, and `localbolt` state docs realigned to current npmjs package pins. Remaining follow-up: run hosted deploy evidence checks for Netlify/Fly.io before closing. |
