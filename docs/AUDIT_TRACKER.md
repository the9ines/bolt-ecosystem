...
|----|--------------------|-------|---------------------|--------|
| EA1 | 24-bit SAS with no commitment is offline-grindable; an active MITM at the rendezvous forces both peers to see a matching "verified" code. `bolt-core-sdk/rust/bolt-core/src/sas.rs:77` (+constants), `localbolt-v3/…/bolt-core-browser/src/sas.ts:77`, `…/webrtc/HandshakeManager.ts:203`, `bolt-protocol/PROTOCOL.md` §SAS. **Reopens SA11.** | PROTOCOL | PAIRING-COMMIT-1 | **RED-TEAMED PASS 1 (2026-07-14): HAS-BLOCKERS (10) + EXTERNAL CRYPTOGRAPHER REVIEW REQUIRED.** The commit-reveal design is ALSO broken: it commits only the ephemeral, so an active MITM runs the ephemeral commit-reveal honestly then birthday-grinds the UNCOMMITTED Ed25519 identity (~2^15-2^16 offline) until both displayed SAS collide — anti-grind FAILS; more bits do not help (offline birthday search). Deeper: current signaling sends the ephemeral in the FIRST frame BEFORE any commit (commitment vacuous); honest Rust vs TS peers ALREADY compute DIFFERENT SAS (caps intersected local-order session.rs:185 vs remote-order HandshakeManager.ts:135 — latent interop BUG independent of EA1); pins are untyped (no v1->v2 migration, upgrade fires KEY_MISMATCH fleet-wide); one-click 'Mark Verified' has NO comparison (collapses the bound to ~2^-1); GOVERNANCE contradiction (SECURITY_MODEL.md says Ed25519, code is X25519, ROADMAP.md recorded a DECISION REJECTING Ed25519 migration; a prior audit row 'verified' an Ed25519 role-separation that does not exist). Corrected design = 9 phases (governance -> external cryptographer design-freeze -> single canonical TLV codec + golden vectors -> commit-before-reveal signaling restructure -> SIGMA-style role-bound Ed25519 sigs + key-confirmation -> channel-bound HKDF SAS -> typed identity pins + re-verify UI -> forced human comparison + fatigue backoff -> conformance/parity CI -> staged rollout). **RECOMMENDATION: do NOT hand-roll; bring in a professional cryptographer AND/OR adopt a vetted construction (ZRTP-done-right, or a PAKE / Magic-Wormhole-style typed code).** Report `docs/evidence/EA1_REDTEAM.md`; PAKE library evaluation: `docs/evidence/PAKE_EVAL.md`. Near-term hole-closing (deny-by-default, honest labels, NO fake 'verified') proceeds independent of EA1. **Honest-labels / no-fake-'verified' step DONE as EA29 (2026-07-15): products no longer claim "Verified", persist a verified pin, or auto-trust on reconnect; real device verification stays blocked on EA1.** |
| EA2 | Attacker-settable `legacy` HELLO flag strips identity binding, SAS, and trust on native WS; the transfer policy then permits no-SAS legacy sessions. `bolt-daemon/src/session_loop.rs:1053,530,599`, `bolt-core-sdk/rust/bolt-app-core/src/contracts/session_contract.rs:90`. Daemon side of SA10. | TRANSPORT | TRUST-GATE-CENTRALIZE-1 | DONE-VERIFIED (daemon side) — bolt-daemon `5022961`: both WS legacy branches now fail closed with an explicit `[PROTOCOL_VIOLATION]`. Answerer (`handle_connection`, was :1053) rejects a `legacy:true` HELLO BEFORE sending any legacy response or building a SessionContext — no more `session.connected`, no `run_session_with_outbound`; offerer (`connect_to_remote_ws`, was :530) rejects a `legacy:true` HELLO *response* instead of running with a zero identity key + empty SAS. The daemon has NO separate transfer-allow gate (no `is_transfer_allowed` caller exists in the daemon), so rejecting at the HELLO gate — strictly before transfer — fully closes the daemon path. Adversarial tests: `ws_legacy_hello_rejected_no_session` (inbound: daemon closes, no legacy response, no session IPC) + `ws_client_rejects_legacy_hello_response` (outbound: `PROTOCOL_VIOLATION` + no session); **mutation-verified** — restoring either legacy-accept fails the matching test. Full daemon lib 259/259 default, 290/290 native-full. Authorization gating only (no verified/pin). NOTE: the `session_contract.rs:90` reference is the bolt-app-core (web SDK) contract layer = SA10 scope, out of this daemon-only change. |
| EA3 | WebTransport session handler never calls `enforce_session_trust`; the preferred browser↔native transport is ungated. `bolt-daemon/src/wt_endpoint.rs:172-305`. | TRANSPORT | TRUST-GATE-CENTRALIZE-1 | DONE-VERIFIED — bolt-daemon `2ae6af7`: WtEndpointConfig gains `trust_config` (threaded from main.rs like WS/QUIC); handle_incoming_session calls the Answerer `enforce_session_trust` after the HELLO/identity exchange and before run_session_with_outbound; missing config denies (item 2 fail-closed). **CODE RED-TEAM (2026-07-14, `docs/evidence/EA3_REDTEAM.md`): production gate CONFIRMED correct across ~30 bypass/fail-open/policy checks, but the original tests did NOT pin it** (deny test tripped only on a 5s timeout, allow test was pass-through, no policy-driven-denial test) — a wrong-role or gate-delete regression would have shipped green. STRENGTHENED (bolt-daemon `e0bfdac`, 2026-07-14): the missing-config deny test now drives a full handshake + real `FileChunk` and asserts the `[PAIRING_DENIED] ... WT_SESSION` cause + no transfer + `ACTIVE_SESSION` None; NEW differential `wt_session_deny_policy_blocks_established_peer` (valid `Deny` policy). **Mutation-verified:** gate-delete → both deny tests fail; role Answerer→Offerer → deny-policy test fails (missing-config test structurally cannot). WT lib 3/3, full daemon lib 265/265, wt_endpoint clippy-clean. Authorization gating only (no verified/pin). Residual (out of scope): the `ACTIVE_SESSION` single-global race is now tracked as EA27. |
| EA4 | Native app spawns the daemon with `--pairing-policy allow` on a `0.0.0.0` WS listener → any LAN host silently writes files into `~/Downloads` (no prompt, no SAS). `localbolt-app/native/shared/src/daemon.rs:75,91`. | NATIVE | NATIVE-PAIRING-ASK-1 (merges into TRUST-GATE-CENTRALIZE-1) | DESIGN-LOCKED — Evan chose the COMPLETE fix (2026-07-14): not just `allow`->`ask` (which denies unpinned inbound; the interactive path is unwired) but wire the prompt. Machinery exists: `check_pairing_approval` (trust.rs:330) emits `pairing.request` + blocks on `server.await_decision`; the Swift `PairingRequestView` answers via `pairing.decision`. Gap: no production caller, and `enforce_session_trust`'s Ask branch (session_loop.rs:115-123) only passes already-pinned identities. FIX: thread `Option<&IpcServer>` (or an approval capability) through run_ws_endpoint -> handle_connection -> enforce_session_trust (called at :562/:698/:818/:1150), and in the Answerer+Ask+unpinned branch emit an IDENTITY-aware pairing.request, await the decision, pin on AllowAlways / allow-once on AllowOnce / deny otherwise. Same code as EA2/EA3 -> do as ONE trust-gate-spine workstream. Also flip daemon.rs:91 `allow`->`ask`. **RED-TEAM PASS 1 (2026-07-14): HAS-BLOCKERS — design `600212f` REJECTED as written.** 10 blockers / 17 high. Core: (a) will NOT compile — IpcServer holds a std::sync::mpsc::Receiver so it is !Send, cannot cross tokio::spawn; (b) net-new DoS — await_decision is a blocking 30s sleep-poll that starves tokio workers, and its single receiver discards mismatched request_ids so concurrent pairings cross-cancel; (c) the pin/'verified' step is UNSOUND and strictly WORSE than today pre-EA1 — remote identity has NO proof-of-possession (X25519, never signs; HELLO identity is a plaintext field) and the 24-bit SAS is grindable, so a MITM launders a forged identity into a permanent 'verified' pin; (d) daemon has NO transfer-time gate (is_transfer_allowed has zero daemon callers), enforce_session_trust returns Ok when trust_config is None (silent-open) and rubber-stamps the Offerer path; (e) the shipped pairing UI only emits allow_once/deny_once so the 'AllowAlways->pin' branch is dead. **REVISED ORDER:** (1) **EA1** = SAS commit-reveal + >=30-bit + transcript binding + identity proof-of-possession (the real foundation; blocks all verified/pin/persistent behavior); (2) rebuild the decision channel async + Send/Sync (one reader task routing by request_id to per-request tokio oneshots, awaited with tokio::time::timeout + is_ui_connected fast-path); (3) EA4-authorization = deny-by-default + honest fail-closed prompt ('a device wants to connect; this does not prove it is your peer'), NO 'verified' pin, + inbound per-IP/concurrency caps landing WITH the prompt; (4) EA2/EA3 bypass-close in parallel (BOTH legacy branches by deletion, fail-CLOSED on absent trust_config, gate WT at run_session_with_outbound before transfer); (5) EA4-authentication (verified pin) AFTER EA1. Full report `docs/evidence/EA4_REDTEAM.md`. **NEAR-TERM STEP DONE (localbolt-app `4c78799`, 2026-07-14):** flipped the native spawn arg `allow`->`ask` (deny-by-default) via a testable `daemon_spawn_args` helper; unit test asserts `ask` not `allow` (mutation-verified). This is EA4-AUTHORIZATION only (deny unpinned inbound) — the interactive prompt and any 'verified'/pin behavior remain DESIGN-LOCKED pending EA1. Status stays OPEN for the full fix. |
...
| EA29 | Products SHIP a false "verified" state pre-EA1: a session is shown "Verified" / "Mark Verified" and a `verified:true` pin is persisted + silently re-asserted on reconnect (SAS skipped), all from a user button click after eyeballing a grindable SAS — no cryptographic device verification exists (that is blocked on EA1). Web: `localbolt-v3/packages/localbolt-browser/{components/verification-status.ts, services/webrtc/HandshakeManager.ts, services/identity/pin-store.ts}` (published to self-hosted `localbolt`). macOS: `localbolt-app/native/macos/Sources/LocalBolt/{BoltBridge.swift, LocalBoltApp.swift}`. Canonical enum `VerificationState::Verified` in bolt-core-sdk. Near-term step of EA1 ("honest labels, NO fake 'verified'"). | PRODUCT | HONEST-VERIFY-1 | DONE (item-6, honest-no-rename; PM-scoped 2026-07-15) — made products honest WITHOUT the breaking canonical enum rename (internal `verified` name kept; rename deferred to a typed-future migration). **Web (localbolt-v3 `14db116`):** "Verified" → "Approved for this session"; "Mark Verified" → "Approve this session"; `markPeerVerified`/`markVerified` persist nothing; `verifyPinnedIdentity` ignores a stored `verified:true`; a key-continuity match never auto-promotes on reconnect. Mutation-verified (reconnect-no-auto-verify, stored-verified-ignored, no-persist, wording); suites browser 353 / core 106 / web 75. **macOS (localbolt-app `c28dcb8`):** `PinStore.isVerified`→false, `markVerified` no-op, reconnect auto-verify branches removed, `checkMismatch` keys on any pin, "Verified" badge → "Approved", "I Verified" → "Approve this session". `PinStore` XCTests added; app compiles + links (XCTest needs Xcode, not runnable in the CLT-only env). Retired surfaces (Tauri, bolt-ui/bolt-ui-wasm) untouched per PM; bolt-daemon already honest (audited: no session-verified state, no prod pin write); self-hosted `localbolt` inherits via the SDK dependency (no source change). Real device verification stays DEFERRED to EA1/PAKE. RESIDUAL OPEN: canonical `VerificationState` enum still literally names `Verified` (rename deferred); EA5 (browser pin keyed on peer code not identity key) unchanged. |
