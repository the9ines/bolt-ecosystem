...
> Counts are NOT maintained here — they rot. (This block was a stale hand-count claiming
> Total 112 / OPEN 1, while dozens of rows below carry OPEN.) Derive current status from the
> rows: open findings = rows whose Status is `OPEN` or `IN-PROGRESS`; count with a token grep
> over the Status column. A generated rollup will land in `os/DASHBOARD.md` Signals once the
> controlled status vocabulary ships (Governance OS v2 Phase 2+; see
> `os/log/decisions/2026-07-15-governance-os-v2-design.md`). The dated "Arithmetic reconciled
> in ecosystem-vX …" lines below are historical records, not current claims.
...
| SA10 | HELLO timeout silently downgrades to unauthenticated legacy mode after 5s (`WebRTCService.ts:433-446`) | TRANSPORT | **DONE-VERIFIED** | TRANSPORT-HARDEN-2 | `sdk-v0.5.12-transport-harden-2` (`ad8cd3c`). HELLO timeout fails closed: disconnect + `onError` callback; no legacy downgrade when identity configured. `WebRTCService.ts` modification. 3 tests in `sa10-hello-timeout-downgrade.test.ts`. ADVERSARIAL: timeout with identity → disconnect, no silent fallback. Final transport-web: 199 tests. Daemon side completed by EA2 (2026-07-15, bolt-daemon `5022961`) — see the EA2 row. |
| SA11 | Identity key not cryptographically bound to ephemeral key in HELLO | PROTOCOL | **DONE-BY-DESIGN** → SUPERSEDED-BY:EA1 | N/A (spec) | PROTOCOL.md §3 defines SAS computation binding identity + ephemeral keys via `SHA-256(sort32(identity_A, identity_B) \|\| sort32(ephemeral_A, ephemeral_B))`. §15.2 documents this as the v1 mitigation. v2 may add transcript hash binding. Accepted design. **REOPENED by EA1** (2026-07-15): the v1 SAS-binding mitigation does not stop offline grinding — see the EA1 row + `os/log/decisions/2026-07-15-ea1-adopt-pake-direction.md`. |
...
|----|--------------------|-------|---------------------|--------|
| EA1 | 24-bit SAS with no commitment is offline-grindable; an active MITM at the rendezvous forces both peers to see a matching "verified" code (`bolt-core-sdk/rust/bolt-core/src/sas.rs:77`, `localbolt-v3/…/webrtc/HandshakeManager.ts:203`, `bolt-protocol/PROTOCOL.md` §SAS). Reopens SA11. | PROTOCOL | PAIRING-COMMIT-1 | **OPEN** — a hand-rolled SAS commit-reveal is also broken (red-teamed twice, HAS-BLOCKERS: the uncommitted identity is offline birthday-grindable). Direction: adopt a vetted PAKE with an external cryptographer; blocks all verified/pin behavior. Evidence: `docs/evidence/EA1_REDTEAM.md`, `docs/evidence/PAKE_EVAL.md`, `docs/evidence/EA1_PAKE_PROFILE_REDTEAM.md`. Decision: `os/log/decisions/2026-07-15-ea1-adopt-pake-direction.md`. **PAKE v1 profile PROPOSED 2026-07-15 — design-only, NOT implementation-authorized, NOT wire-frozen** (`os/log/decisions/2026-07-15-ea1-pake-v1-profile-proposal.md`); adversarial red-team returned **HAS-BLOCKERS** (1 BLOCKER = no identity proof-of-possession → zero-guess reconnect MITM; +3 HIGH/3 MEDIUM/3 LOW; 9 required design changes before any wire-freeze). **v2 revised draft** addressing the 9 changes (identity proof-of-possession Noise-DH schedule + reconnect static-DH rule; consumption/single-flight/lockout; ceremony-bound floor; structural routing/secret split; injective codec; CPace-vs-SPAKE2 open; fold-locally channel binding; widened formal model; contact_id pins + honest non-"verified" states): `os/log/decisions/2026-07-15-ea1-pake-v2-profile-draft.md` (design-only). **v2 red-team 2026-07-16 = HAS-BLOCKERS** (`docs/evidence/EA1_PAKE_V2_REDTEAM.md`): 1 BLOCKER (low-order X25519 point makes DH return all-zero → forges the identity possession proof → reopens the reconnect MITM) + 2 HIGH + 4 MEDIUM confirmed, but design direction validated and 13 items correctly deferred to the external cryptographer; v2 marked **PROPOSED — REVISION REQUIRED** with 8 required changes before wire-freeze. **v3 revised draft** incorporating all 8 changes — new §0 key-validity (reject small-order Curve25519 u-coords + abort all-zero X25519 DH, RFC 7748; identity+ephemeral, pairing+reconnect), conforming-client routing/secret split, `session_root` transcript binding + ephemeral==BTR-root MUST, primitive-independent anti-reflection, white-box CSPRNG KAT, non-overridable reconnect floor from `possession_proven` pins, CSPRNG `contact_id` + human-confirmed rotation, authenticated-only code consumption; retracts the falsified v1/v2 claims: `os/log/decisions/2026-07-16-ea1-pake-v3-profile-draft.md` (design-only). **v3 red-team 2026-07-16 = NEEDS-REVISION** (`docs/evidence/EA1_PAKE_V3_REDTEAM.md`): NO blocker — §0 genuinely closes the v2 low-order BLOCKER and the reconnect downgrade floor holds; 1 HIGH + 4 MEDIUM confirmed **draft-defects** (§8 backoff-DoS self-contradiction; §6 session_root public-salt option + undefined ephemeral_shared_secret; §1 code-verified-false legacy-parser claim; §4 false reflection-independence lemma) + 11 items deferred to the cryptographer; v3 marked **PROPOSED — NEEDS REVISION** with 9 required draft edits before wire-freeze. **v4 revised draft** incorporating all 9 edits — decoupled §8 consume-vs-device-backoff (backoff never from anonymous inbound), `session_root = HKDF(salt = PRK)` (no public salt), `ephemeral_shared_secret = ee` well-typed, structural separate ROUTING/SECRET API inputs, joint anti-reflection with load-bearing direction-separated confirmation, canonical X25519 ingress rejection, differential CSPRNG KAT, reconnect `contact_id` resolution + precise KEY_MISMATCH, rotation gated behind CD1b: `os/log/decisions/2026-07-17-ea1-pake-v4-profile-draft.md` (design-only). **v4 red-team 2026-07-17 = NEEDS-REVISION** (cryptographer-ready: No; `docs/evidence/EA1_PAKE_V4_REDTEAM.md`): NO blocker, 7/9 v3-edits landed clean; 2 confirmed draft-defects block handoff — §6 PRK-salted `session_root` is disconnected from the normative data schedule (data keys from `ee`; pick fork A=re-root the schedule via a future PROTOCOL.md delta, or B=re-attribute to ee-auth) + §9 inbound KEY_MISMATCH is attacker-summonable via the stable `reconnect_handle` (split by initiator) — plus 8 LOW cleanups + 6 cryptographer decisions; v4 marked **PROPOSED — NEEDS REVISION** (v5 revision pending, expected ACCEPTABLE-FOR-CRYPTOGRAPHER-REVIEW after the fixes). **v5 revised draft** adopts **§6 fork A** (post-handshake data/BTR keys derive from the authenticated `session_root = HKDF(salt=PRK, ikm=ee, info=TT/domain)`, not bare `ee`) — captured as a "Required future PROTOCOL.md delta" (NO PROTOCOL.md edit in the gate; cryptographer/formal model reviews the PROPOSED schedule, not the old ee-rooted wire) — plus all 10 v4-review edits (KEY_MISMATCH split by initiator; FIX-8/9 reconciled; honest code-burning DoS; formal-model byte-layer cleanup; RFC 7748 citation; reconnect confidentiality chain; narrowed SECRET-off-server; pinned §5 L/R basis; sort-discriminating es/se vectors; reconnect_handle invariants): `os/log/decisions/2026-07-17-ea1-pake-v5-profile-draft.md` (design-only; 8 cryptographer decisions marked). **v5 red-team 2026-07-17 = NEEDS-REVISION** (cryptographer-ready: No, two author-text edits away; `docs/evidence/EA1_PAKE_V5_REDTEAM.md`): NO blocker — **fork A legitimately closes the v4 HIGH** (session_root now consumed, no shadow value; future PROTOCOL.md delta clearly flagged as proposed not shipped). The only blockers are 2 MEDIUM draft-defects on the KEY_MISMATCH surface — (CONFIRMED) [FIX 2]'s "reconnect_handle cannot summon a forged alert" is false (untrusted rendezvous redirects a locally-initiated reconnect → hostile contact-key-changed verdict about an honest contact; narrow the claim or commit strict-KK) + (PLAUSIBLE) FIX-1's rate-limiter scope is unpinned → reconnect-DoS (add a §9 no-throttle-handshake-completing invariant) — plus 5 LOW cleanups + 8 cryptographer decisions; v5 marked **PROPOSED — NEEDS REVISION** (v6 revision pending). **v6 revised draft** adds a class-level **§AV Adverse-Verdict Invariant** (no keyless/SECRET-less party — incl. untrusted rendezvous — may cause any adverse user-visible verdict / pin mutation / contact-key-changed warning / re-pair prompt / contact-sticky throttle vs an honest possession_proven contact on ANY path; redirected locally-initiated reconnect → neutral TAMPER/UNREACHABLE not KEY_MISMATCH; key-change verdict requires prior-key possession → CD1b; never mutate pin on a keyless failure) — replacing path-by-path patching — plus both v5 MEDIUM fixes + 6 LOW cleanups; fork A retained: `os/log/decisions/2026-07-17-ea1-pake-v6-profile-draft.md` (design-only). **v6 red-team 2026-07-17 = NEEDS-REVISION** (cryptographer-ready: No, one localized text fix away; `docs/evidence/EA1_PAKE_V6_REDTEAM.md`): NO blocker — **§AV holds on all paths and the hostile key_mismatch removal is a net improvement, not a regression** (the recurring KEY_MISMATCH class is closed at the class level); fork A stays not-shipped + shadow-free. The ONLY handoff-gating defect is 1 CONFIRMED MEDIUM draft-defect — the v6 §5 "K_session retirement" over-reached and falsely denies the canonical ratcheting session_root_key (contradicts PROTOCOL.md §16.3 + the draft's own delta; literal reading collapses per-transfer FS; fix = two-level "session_root SEEDS the gen-0 BTR session_root_key, ratchet + BTR-INV-01..11 retained, only the gen-0 seed changes ee→session_root") — plus 3 LOW editorial cleanups + 7 cryptographer decisions; v6 marked **PROPOSED — NEEDS REVISION** (v7 revision pending). **v7 revised draft** corrects the sole v6 MEDIUM — fork A **re-seeds** the BTR ratchet rather than flattening it: `session_root` seeds the gen-0 BTR `session_root_key = HKDF(ikm=session_root, info="bolt-btr-session-root-v1")` (replacing the current PROTOCOL.md §16.3 salt=EMPTY/ikm=ee seed); the BTR hierarchy + inter-transfer DH ratchet + BTR-INV-01..11 + per-transfer FS are RETAINED; only the gen-0 seed changes ee→session_root; K_session (§5 flat orphan) stays retired, distinct from session_root_key — plus the 4 v6 LOW cleanups (rate-limiter honesty + pre-DH availability disclosure→CD2a; es/se wire-role negative; clamped-X25519 on ee/es/se/ss; exporter/tier-select pin restored into capabilities[]/TT); §AV + fork A retained: `os/log/decisions/2026-07-18-ea1-pake-v7-profile-draft.md` (design-only). **v7 red-team 2026-07-18 = ACCEPTABLE-FOR-CRYPTOGRAPHER-REVIEW** (cryptographer-ready: Yes; `docs/evidence/EA1_PAKE_V7_REDTEAM.md`): NO blocker, no confirmed HIGH/MEDIUM draft-defect; all 3 primary checks verified line-by-line vs `PROTOCOL.md §16.3` (fork-A BTR re-seed correct — ratchet RETAINED not flattened; §AV holds; remaining items are cryptographer/formal decisions). Seven-pass convergence: v1(BLOCKER)→v2(BLOCKER)→v3(9 edits)→v4(2)→v5(2)→v6(1)→**v7 ACCEPTABLE**. Two remaining draft-defects (§1 hosted-product SECRET-egress MUST; §0 5th-DH obligation #3↔#6 alignment) are **pre-wire-freeze, not pre-handoff**. **EA1 is DESIGN-COMPLETE for external cryptographer + formal-methods review — NOT implementation-ready**; wire-freeze remains gated on the resolved `[CRYPTO-DECISION]` set (CD1a–CD7a), the authored+authorized future `PROTOCOL.md` delta, the cryptographer sign-off, and a passing formal model (obligations #0–#10). Stays **OPEN**; PAKE remains design / spec-drafting only — implementation and wire-freeze forbidden pending the revision, the resolved cryptographer decisions, external sign-off, and a formal model. |
| EA2 | Attacker-settable `legacy` HELLO flag strips identity binding, SAS, and trust on native WS (`bolt-daemon/src/session_loop.rs`). Daemon side of SA10. | TRANSPORT | TRUST-GATE-CENTRALIZE-1 | **DONE-VERIFIED** — both WS legacy branches fail closed with `[PROTOCOL_VIOLATION]` (bolt-daemon `5022961`; two adversarial tests, mutation-verified). Authorization gating only. Decision: `os/log/decisions/2026-07-15-trust-gate-and-honest-verification.md`. |
| EA3 | WebTransport session handler never called `enforce_session_trust`; the preferred browser↔native transport was ungated (`bolt-daemon/src/wt_endpoint.rs`). | TRANSPORT | TRUST-GATE-CENTRALIZE-1 | **DONE-VERIFIED** — WT now passes the same Answerer trust gate as WS/QUIC, fail-closed on missing config (bolt-daemon `2ae6af7`); tests hardened + mutation-verified after a code red-team (`e0bfdac`). Evidence: `docs/evidence/EA3_REDTEAM.md`. Decision: `os/log/decisions/2026-07-15-trust-gate-and-honest-verification.md`. Residual: EA27. |
| EA4 | Native app spawns the daemon with `--pairing-policy allow` on a `0.0.0.0` WS listener → any LAN host silently writes files into `~/Downloads` (no prompt, no SAS). `localbolt-app/native/shared/src/daemon.rs`. | NATIVE | NATIVE-PAIRING-ASK-1 (merges into TRUST-GATE-CENTRALIZE-1) | **DEFERRED** (full fix pending EA1) — near-term authorization step is DONE-VERIFIED: native launches with `--pairing-policy ask` not `allow` (localbolt-app `4c78799`, mutation-verified). The full interactive-prompt + verified-pin design was red-teamed HAS-BLOCKERS and is design-locked behind EA1. Evidence: `docs/evidence/EA4_REDTEAM.md`. Decisions: `os/log/decisions/2026-07-15-trust-gate-and-honest-verification.md`, `os/log/decisions/2026-07-15-ea1-adopt-pake-direction.md`. |
...
| EA8 | Daemon identity key + TOFU store live in a predictable world-writable `/tmp/bolt-native-<pid>` dir, regenerated every launch (degrades TOFU/MITM protection). `localbolt-app/native/shared/src/daemon.rs:72-82`; harden `bolt-daemon/src/identity_store.rs` (reject symlink parent, verify uid, O_EXCL 0600). Fix: use `bolt_app_core::platform::default_data_dir()`. | SECURITY | NATIVE-KEYDIR-1 | PARTIAL — native-app keydir DONE (localbolt-app `9077eb4`, 2026-07-14): `--data-dir` now points at `bolt_app_core::platform::default_data_dir()` (persistent per-user path, e.g. `~/Library/Application Support/LocalBolt/daemon`), so identity + TOFU pins survive restarts instead of regenerating each launch; `bolt_daemon_stop` no longer deletes the now-persistent store, only the ephemeral `/tmp` IPC socket; unit test asserts data_dir is the platform default and never `/tmp/bolt-native-<pid>` (mutation-verified). Bridge tests 23/23. REMAINS OPEN: the `bolt-daemon/src/identity_store.rs` hardening (reject symlink parent, verify uid, O_EXCL 0600) — separate repo/sub-item. |
| EA9 | Interior-NUL peer metadata → `CString::new().unwrap()` aborts the native app (zero-interaction remote DoS). `localbolt-app/native/shared/src/signaling.rs:214`; sanitize in `bolt-app-core/…/signaling_client.rs:62`. | AVAILABILITY | FFI-PANIC-SAFETY-1 | DONE-VERIFIED — localbolt-app `6cc4533` (null-ptr on a NUL-bearing peer field, mirroring wt_url; 21/21 native-bridge tests incl. 2 adversarial). |
| EA10 | Bundled QUIC/WT stack ships quinn-proto 0.11.14 (RUSTSEC-2026-0185): remote memory-exhaustion DoS. `bolt-daemon/Cargo.lock:978`. Fix: `cargo update -p quinn-proto` (≥0.11.15) + quinn TransportConfig stream/window caps. | SUPPLY-CHAIN | DEP-QUINN-1 | DONE-VERIFIED — bolt-daemon `2af6066` (0.11.14->0.11.15; cargo-audit clears RUSTSEC-2026-0185; 281/281 serialized lib tests; CI never enables transport-quic). Optional TransportConfig receive-window caps deferred. |
...
### Follow-on findings (discovered during EA remediation, 2026-07-14)

| ID | Finding & location | Severity | Proposed workstream | Status |
|----|--------------------|----------|---------------------|--------|
| EA25 | A fresh `cargo-audit` (run while verifying EA10) surfaces further advisories in the bundled TLS/cert stack: rustls-webpki reachable panic parsing CRLs (RUSTSEC-2026-0104), name constraints wrongly accepted for URI names (RUSTSEC-2026-0098) and for wildcard names (RUSTSEC-2026-0099), CRLs not treated as authoritative by distribution point (RUSTSEC-2026-0049); plus `anyhow` `Error::downcast_mut()` unsoundness (RUSTSEC-2026-0190) and `rand` custom-logger unsoundness (RUSTSEC-2026-0097). Transitive via quinn/wtransport/rustls in `bolt-daemon/Cargo.lock`. Likely surgical `cargo update` bumps like EA10. | MEDIUM | DEP-AUDIT-1 | DONE-VERIFIED — bolt-daemon `e3feb0f` (anyhow 1.0.103, rand 0.8.6 + 0.9.3, rustls-webpki 0.103.13; non-cascading, 0 crates added/removed; cargo-audit clears all 6; cargo check clean). |
| EA26 | Pre-existing test-isolation defect: the process-global `ACTIVE_SESSION` / `IPC_TX` singletons (`bolt-daemon/src/session_loop.rs:172,207`) are written by production session code and shared across every parallel session test, so `cargo test --features transport-quic` (or `native-full`) fails ~1-2 tests under the default parallel runner. Independent of EA10 — baseline 0.11.14 fails 6/6 identically; passes 281/281 single-threaded. CI unaffected (default = transport-ws). Proper fix: serialize the whole session-test group or make the session registry injectable (a partial two-test guard is insufficient). | LOW | TEST-ISOLATION-1 | DONE-VERIFIED — bolt-daemon `997902d`. CORRECTION: the earlier "CI unaffected (default = transport-ws)" note was WRONG — it flaked under DEFAULT `cargo test` parallel too (reproduced: baseline dc96976 2/4, HEAD 0d5cfba 5/6). Fix: test-only `SESSION_GLOBALS_LOCK` (tokio::sync::Mutex) that the 10 session-establishing tests (7 WS + 3 QUIC) hold; production unchanged (single native session). After: default WS parallel 10/10, transport-quic session 5/5. |
| EA27 | Pre-existing production concurrency race surfaced by the item-3 WT code red-team: `ACTIVE_SESSION` is a single process-global `Mutex<Option<ActiveSessionHandle>>` (`bolt-daemon/src/session_loop.rs:182`). Registration in `run_session_with_outbound` does `*guard = Some(..)` with NO existing-session check (last-writer-wins); teardown clears it unconditionally (`*guard = None`). With WS/WT/QUIC each spawning an unbounded task per accept, two concurrent sessions clobber each other: session B's registration overwrites A so a native send meant for A streams to B; or A's teardown nulls the slot while B is live, killing B's outbound. NOT an auth bypass (the per-session trust gate still runs) and NOT introduced by EA3 (item 3 strictly SHRINKS the window — a denied peer never registers). The EA26 `SESSION_GLOBALS_LOCK` MASKS it in CI, so "session tests parallel N/N" is auth-correctness evidence only, NOT concurrency-safety evidence. | HIGH | SESSION-REGISTRY-1 (TBD) | OPEN — needs compare-and-set registration (reject or atomic-replace) + compare-and-clear teardown, or a per-session registry keyed by session id; plus a concurrency test that drives two sessions WITHOUT the lock. Trust-gate items must NOT rely on the single-active-session invariant. Discovered by the WT code red-team (`docs/evidence/EA3_REDTEAM.md`). |
| EA28 | The WebTransport session path omits the `session.connected` + `session.sas` IPC events that the WS and QUIC paths emit, so a WT session surfaces via transfer events but not via connection/SAS events (`bolt-daemon/src/wt_endpoint.rs` vs the WS/QUIC emit sites). Observability / verification-surface gap, PRE-EXISTING, not an auth bypass. | MEDIUM | WT-IPC-PARITY-1 (TBD) | OPEN — emit the same `session.connected` / `session.sas` events from the WT handler as WS/QUIC once past the trust gate. Discovered by the WT code red-team. |
| EA29 | Products SHIP a false "verified" state pre-EA1 — a session shows "Verified"/"Mark Verified", persists a `verified:true` pin, and silently re-asserts it on reconnect, all from a button click after eyeballing a grindable SAS (web `localbolt-v3` + macOS `localbolt-app`; canonical `VerificationState::Verified` in bolt-core-sdk). Near-term step of EA1. | PRODUCT | HONEST-VERIFY-1 | **DONE-VERIFIED** — products no longer claim "Verified", persist a verified pin, or auto-trust on reconnect; honest-no-rename (localbolt-v3 `14db116`, localbolt-app `c28dcb8`; mutation-verified). Real device verification stays deferred to EA1. Decision: `os/log/decisions/2026-07-15-trust-gate-and-honest-verification.md`. Residual: canonical enum still named `Verified` (rename deferred); EA5. |
