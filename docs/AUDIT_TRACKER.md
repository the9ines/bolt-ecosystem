...
| EA3 | WebTransport session handler never calls `enforce_session_trust`; the preferred browser↔native transport is ungated. `bolt-daemon/src/wt_endpoint.rs:172-305`. | TRANSPORT | TRUST-GATE-CENTRALIZE-1 | DONE-VERIFIED — bolt-daemon `2ae6af7`: WtEndpointConfig gains `trust_config` (threaded from main.rs like WS/QUIC); handle_incoming_session calls the Answerer `enforce_session_trust` after the HELLO/identity exchange and before run_session_with_outbound; missing config denies (item 2 fail-closed); adversarial deny test + explicit-allow test; WT 6/6 single-threaded, native-full session+WT parallel 5/5. Authorization gating only (no verified/pin). |
