...
> **Status:** Normative
> **Created:** 2026-03-02
> **Updated:** 2026-04-09 (WEB-SURFACE-CONSOLIDATION-1 CLOSED — AC-6 SEO copy deployed, all acceptance criteria met)
...
| Phase | Description | Type | Serial Gate | Dependencies | Status |
|-------|-------------|------|-------------|--------------|--------|
| **RU1** | Reliability/UX audit + prioritized issue list | Audit | YES — gates RU2 | None | **DONE** (`ecosystem-v0.1.180`, 2026-03-19). 10 issues, 4 HIGH. Fix proposals defined. |
| **RU2** | Transfer-state visibility hardening | Engineering | YES — gates RU4 | RU1 complete | **DONE** (2026-03-19). Connection phase, pause label, completion state, SAS guidance. |
...
| **PF5** | Comparative transport assessment (conditional) | Audit/Engineering | NO — optional, PM-PF-01 gated | PF3 + PF4 complete | **DONE** (`ecosystem-v0.1.192`, 2026-03-20). AC-PF-13 PASS (ceiling assessed). AC-PF-14 N/A (no alt transport to compare). |
| **PF6** | Validation + closure | PM gate | YES — closes stream | PF3 + PF4 complete | **DONE** (`ecosystem-v0.1.193`, 2026-03-20). AC-PF-15/16/17 PASS. Stream CLOSED. |
...
> **Supersession note (2026-05-16, LOCALBOLT-LAN-DISCOVERY-RELIABILITY-1):**
> This section is historical and predates the current hosted `localbolt.app`
> discovery posture. Current LocalBolt policy is: the website may use the
> canonical hosted `bolt-rendezvous` endpoint to make same-LAN website users
...
| WEB-SURFACE-CONSOLIDATION-1 | Consolidate web surface: localbolt (canonical app) + localbolt-v3 (site consumer) | P2 | localbolt + localbolt-v3 + ecosystem | **CLOSED** (2026-04-09). All 6 ACs met. IMPL-1/2/3 complete. AC-6 SEO copy deployed (`v3.0.102-seo-copy-refresh`). Netlify Git-triggered deploys restored. Remaining optional: further v3 file convergence, deferred repo rename. |
| LOCALBOLT-NATIVE-QUIT-TEARDOWN-1 | Orphan daemon on native app quit (Cmd+Q) | P1 | bolt-daemon | **CLOSED** (2026-04-26). Daemon-side ppid watchdog in WsEndpoint mode. `libc::_exit(0)` on parent death. Rejected: willTerminateNotification, AppDelegate, atexit, FFI shell watchdog. Commit `bffbc02`. WT disconnect changes excluded (separate task). |
...
### DEWEBRTC-1/2 — WebRTC Retirement from bolt-daemon (CLOSED — COMPLETE)

> **Status:** CLOSED (COMPLETE — Phase 1 extraction + Phase 2 deletion)
> **Closed:** 2026-03-24
> **Commits:** `7f443f4`, `a06e43e` (Phase 1), `f730501` (Phase 2) — bolt-daemon
> **Priority:** P5
> **Dependency:** DAEMON-BTR-1 (CLOSED)

**Phase 1:** Extracted WebRTC code behind `legacy-webrtc` feature gate. Default builds non-WebRTC.

**Phase 2:** Deleted all remaining WebRTC/DataChannel code. 7,757 lines removed. 5 source files + 1 test file deleted. `datachannel` and `webrtc-sdp` removed from all dependency sections. `legacy-webrtc` feature flag removed. bolt-daemon is now fully zero-WebRTC. 320 tests pass.
...
> **Status:** CLOSED (2026-04-09). All 6 acceptance criteria met. IMPL-1/2/3 + AC-6 SEO copy deployed. Netlify Git-triggered deploys restored.
...
The web surface is split across two active repos with overlapping responsibilities:
...
3. **Naming inversion.** The repo called `localbolt` is not the canonical web app authority. The repo called `localbolt-v3` is.
...
**localbolt standalone capability (validated P1):**
- Builds in 459ms, 329 tests pass, 71.73% branch coverage (threshold: 70%)
- Consumes `@the9ines/bolt-core` 0.6.4, `@the9ines/bolt-transport-web` 0.7.14, `@the9ines/localbolt-core` 0.1.2 from npm (aligned with latest published)
- Has all core app features: peer discovery, file transfer, SAS, WASM authority, self-hosted path
- Zero app-behavior gaps vs localbolt-v3

#### Plan Revision History

**Original plan (APPROVED 2026-04-08):** Create `localbolt-web` as an Astro site shell at localbolt.app, embed localbolt as build artifact at `/app/`, sunset localbolt-v3.

**Staging review finding:** The Astro shell was thin and generic compared to localbolt-v3's existing polished site. Rebuilding site/SEO content in a new framework added unnecessary churn without improving the product. The localbolt-v3 site already works well — the problem was authority, not presentation.

**Revised plan (2026-04-08):** No new repo. `localbolt` becomes the canonical app source. `localbolt-v3` stays as the site/deployment layer, consuming localbolt. Repo rename deferred to pre-launch.

**Actions taken:**
- `localbolt-web` GitHub repo deleted
- `localbolt-web` local directory deleted
- Netlify draft deploy cleaned up (no production change was made)
- P1 and P1-FIX work retained (localbolt package alignment + coverage gate — valuable regardless of approach)

#### Target Architecture (Revised)

```
localbolt          → canonical web app behavior (React + TS + WASM)
                     329 tests, package-aligned with latest published
                     self-hosted: `start.sh` / `start.bat` preserved
                     source of truth for app logic

localbolt-v3       → site layer + deployment + package source
                     domain: localbolt.app (Netlify, unchanged)
                     owns: SEO, marketing content, site chrome
                     publishes npm packages consumed by localbolt
                     consumes localbolt as canonical app authority
```
...
| Self-hosted deployment (`start.sh`) | `localbolt` |
| Hosted site at `localbolt.app` | `localbolt-v3` |
| SEO, marketing copy, site chrome | `localbolt-v3` |
| Netlify config + deploy pipeline | `localbolt-v3` |
| npm packages: @the9ines/bolt-core, bolt-transport-web, localbolt-core | `localbolt-v3` |

#### Remaining Work

1. **Make localbolt-v3's localbolt-web package consume localbolt as canonical** — the app behavior in localbolt-v3's SPA should derive from localbolt, not maintain a parallel implementation.
2. **Update SEO/marketing copy** — current copy on localbolt.app is outdated for the project's current state (PM task).
3. **Repo rename** — `localbolt-v3` → `localbolt-web` when the project is ready for launch. Deferred to reduce friction.

#### Out-of-Scope

- Native app changes
- Protocol or SDK changes
- New web features
...
| AC-6 | SEO/marketing copy updated for current project state | **DONE** (LOCALBOLT-SEO-COPY-1: `v3.0.102-seo-copy-refresh`, `9a6667a`. Deployed to production 2026-04-09) |

#### Decision

**CLOSED (2026-04-09).** All acceptance criteria met. Implementation phases:
...
| SEO-COPY | `9a6667a` | `v3.0.102-seo-copy-refresh` | localbolt-v3 | Refreshed all public-facing copy: title, meta, H1, features, FAQ, trust strip, structured data. Open-source + Bolt Protocol + browser-to-app positioning |
...
**Status: CLOSED (2026-04-09).** All 6 acceptance criteria are met. The engineering consolidation (IMPL-1/2/3), infrastructure recovery (Netlify Git deploys), and content refresh (AC-6 SEO copy) are complete and deployed to production. Remaining optional items (further v3 file convergence, repo rename) are standalone backlog items — they do not reopen this stream.
