...
> **Status:** Normative
> **Created:** 2026-03-02
> **Tag:** ecosystem-v0.1.30-workstreams-1
...
- [ ] New HandshakeManager unit tests pass (report actual count)
- [ ] `npm run build` succeeds (bolt-core + transport-web)
- [ ] No public API changes (export surface identical)
- [ ] Commit + local tag: `sdk-vX.Y.Z-webrtc-decompose-A1`
- [ ] Phase report filed
...
**Goal:** Extract envelope encode/decode logic (seal, open, nonce management, MAC verification) from WebRTCService into a dedicated EnvelopeCodec module.

**Constraints:**
- WebRTCService public API unchanged.
- No cryptographic changes — same NaCl box operations, same nonce generation.
- EnvelopeCodec is internal; not exported from package public surface.

**Allowed files (bolt-core-sdk):**
- `ts/bolt-transport-web/src/envelope-codec.ts` (new)
- `ts/bolt-transport-web/src/webrtc-service.ts` (refactor)
- `ts/bolt-transport-web/src/index.ts` (internal re-exports only)
- `ts/bolt-transport-web/__tests__/envelope-codec.test.ts` (new)
- Existing test files (verify green, no structural changes)

**Gates:**
- [ ] Working tree clean before and after
- [ ] All existing transport-web tests pass (report actual count)
- [ ] New EnvelopeCodec unit tests pass (report actual count)
- [ ] `npm run build` succeeds (bolt-core + transport-web)
- [ ] No public API changes (export surface identical)
- [ ] Golden vector tests still pass (report actual count)
- [ ] Commit + local tag: `sdk-vX.Y.Z-webrtc-decompose-A2`
- [ ] Phase report filed
...
### A4 — Slim WebRTCService to Coordinator
...
- [ ] New B1 tests pass (report actual count)
- [ ] `cargo clippy -- -D warnings` clean
- [ ] `scripts/check_no_panic.sh` passes
- [ ] Default-mode HELLO handshake test passes
- [ ] Legacy-flag HELLO handshake test passes
- [ ] Commit + local tag: `daemon-vX.Y.Z-transfer-converge-B1`
- [ ] Phase report filed
...
- [ ] All existing daemon tests pass: `cargo test` (report actual count)
- [ ] All existing daemon tests pass: `cargo test --features test-support` (report actual count)
- [ ] New B2 parsing tests pass (report actual count)
- [ ] `cargo clippy -- -D warnings` clean
- [ ] `cargo fmt` clean
- [ ] Round-trip serialize/deserialize tests for every message variant
- [ ] Reject-malformed tests for every message variant
- [ ] Commit + local tag: `daemon-vX.Y.Z-transfer-converge-B2`
- [ ] Phase report filed
...
|------------|------|--------|---------|
| A-stream | bolt-core-sdk | `sdk-vX.Y.Z-webrtc-decompose-A{1..5}` | `sdk-v0.5.22-webrtc-decompose-A1` |
